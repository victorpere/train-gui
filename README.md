...
Overview
- `train_gui.py` — Tkinter-based GUI (thin view layer). Accepts injected `buttons_data` and a `TrackModel` instance.
- `track_model.py` — Encapsulates serial I/O, state (direction/target/actual), and a background reader thread. Notifies listeners via callbacks.
...
- Use `fake_serial.FakeSerial` by passing a `serial_factory` to `TrackModel` (see `tests/run_fake_test.py`).

Development notes
- The project separates concerns: UI in `train_gui.py`, logic/serial in `track_model.py`. This makes unit testing and mocking easier.
